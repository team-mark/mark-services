...
const buildMain = () => {
    ...
    const functions = deps.map(d => projectBuilders[d])

    const promises = functions.map(f => {
        return new Promise((resolve) => {
            gulp.series(f, resolve);
        })
    });
    ...
    // return gulp.parallel(...promises); // gulp.parallel(...deps.map(d => projectBuilders[d]));
    return gulp.parallel(...deps.map(d => `ts-${d}`));
};
...
const installMain = () => {
    ...
    const commands = Promise.all(
        deps.map(name => new Promise((resolve) =>
            runCommand('npm install', { cwd: `projects/${name}` }, resolve))
        )
    );
...
projectNames.forEach(project => {
    ...
    projectBuilders[project] = transpileProjects([project]);
});

console.log('builder projects', projectBuilders)
